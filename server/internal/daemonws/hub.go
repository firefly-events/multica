...
type HeartbeatHandler func(ctx context.Context, identity ClientIdentity, runtimeID string, supportsBatchImport bool, tokenStatus string) (*protocol.DaemonHeartbeatAckPayload, error)
...
func (c *client) handleHeartbeatFrame(raw json.RawMessage) {
	...
	ack, err := handler(context.Background(), c.identity, payload.RuntimeID, payload.SupportsBatchImport, payload.TokenStatus)
	if err != nil {
