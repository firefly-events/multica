...
type AgentTaskResponse struct {
	...
	// LastHeartbeatAt (DOS-1042) is the per-task liveness signal: stamped by
	// GetTaskStatus on the daemon's existing ~5s poll (watchTaskCancellation),
	// so it only ever advances while status=="running". nil for a task that
	// never started running, or predates this field. Distinct from the
	// runtime-level daemon_connection.last_heartbeat_at -- a runtime can be
	// online while one specific task on it is hung.
