...
type createMessageRequest struct {
	...
	Model         *string `json:"model"`          // optional model identifier
}
...
func handleCreateMessage(store *Store, hub realtime.Broadcaster) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		msg, err := store.CreateMessage(r.Context(), wsID, req.ThreadID, authorID, req.Body, role, req.TokensUsed, req.ContextWindow, req.Model)
		if err != nil {
