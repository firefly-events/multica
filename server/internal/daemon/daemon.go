...
func (d *Daemon) detectBuiltinRuntimes(ctx context.Context) ([]map[string]string, map[string]runtimeVerdict, map[string]string) {
	...
	for name, entry := range d.agents() {
		...
		g.Go(func() error {
			...
			// Probed on the SAME per-provider goroutine as the version check,
			// bounded by the SAME runtimeVersionProbeConcurrency limit — a
			// cold-cache token probe (up to tokenProbeExecTimeout) for one
			// provider must not serialize behind another's (DOS-1037 review
			// follow-up, see currentTokenStatus doc and DOS-1256). Computing
			// it later in the sequential results-building loop below would
			// silently reintroduce exactly that serialization.
			tokenStatus := d.currentTokenStatus(ctx, name)
			mu.Lock()
			results = append(results, detected{name: name, version: version, tokenStatus: tokenStatus})
			mu.Unlock()
			return nil
		})
	}
	...
	for _, r := range results {
		...
		// verdict is inconclusive — see currentTokenStatus doc (DOS-1037).
		// Registration has no prior status to fall back to, so an inconclusive
		// probe here starts the runtime online rather than leaving it in limbo.
		// Already probed concurrently above, alongside the version check.
...
func (d *Daemon) handlePendingWorkHint(runtimeID, kind string) {
	...
	// "" leaves the server-side token status untouched -- this is a
	// lightweight, purpose-specific heartbeat (a pending-work hint), not the
	// main periodic beat that carries a fresh token-probe verdict.
